...
public class YouTubeControllerImpl extends JFrame implements YouTubeController {
	...
	public void reproduirVideo(int idVideo) {
		Video video=repository.buscarVideo(idVideo);
		if (video == null) {
			System.out.println("Usuari no existeix");
			return;
		}
		
		ReproductorVideo reproductorVideo = new ReproductorVideo(video);
	}
	/**
	 * 
	 * @param nom
	 ...
	 */
	@Override
	public void createVideo(String url, String titol, int idUsuari,int durada)
			throws Exception {
		...
		Video video = new Video(url, titol,durada);
